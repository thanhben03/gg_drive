...
class File extends Model
{
    ...
    public function getRootById($file_id)
    {
        $file = File::query()->where('id', $file_id)->first();
        if (count($file->ancestors) > 1) {

            return $file->ancestors[1];

        }
        return $file->ancestors[0];
    ...
    public function isShareWithRootFolder()
    {

        $fileShares = FileShares::query()
                                ->where('file_id', $this->getRoot()->id)
                                ->firstOrFail();
        return !!$fileShares;
    }
    ...
    {
        return $this->hasOne(FileShares::class, 'file_id', 'id')
            ->where('user_id', Auth::id());
    }
